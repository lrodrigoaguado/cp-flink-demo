...
The demo processes fake data simulating a fleet of trucks. Two Kafka topics are continuously populated:
- `vehicle-info`: real time engine RPM and temperature readings (generated by Datagen).
- `vehicle-location`: real time vehicle coordinates (generated by a custom Python simulator using real OSM road data).
- `vehicle_description`: a static dimension table (80 rows) residing in Postgres. Flink now accesses it directly via the JDBC connector for enrichment.
...
- The second one takes the measures in the vehicle_info topic and in the vehicle_speed topic and generates alerts if any of the values (temperature, RPMs or speed) is above a certain threshold. The data is notional, but the thresholds are tuned so that alerts represent rare exceptions (an occasional speeding episode or sensor spike) rather than constant noise. The alerts are populated into the vehicle_alerts topic.
...
| Control Center    | 2.5.1   | 1        | The web-based management UI          |
...
Start the local Postgres instance (contains the `vehicle_description` dimension table seeded with 80 vehicles):
...
We use two different methods to generate data:
...
* `vehicle-info`: continuous engine stats stream from Datagen.
...
The dashboard is built around a live map of the fleet that auto-refreshes every two seconds:

- **Vehicles (live)** — the latest known position of every truck, updated in place so the markers move smoothly across the map.
- **Alert layers** — alerts plotted where they happen, colour-coded by type: 🔴 engine overheat, 🟠 excessive speed, 🟡 excessive RPM. Hover a marker to see the driver, license plate, brand and the offending value. The map shows only the last couple of minutes of alerts, so they appear and fade as incidents come and go.
